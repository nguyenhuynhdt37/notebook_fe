...
import { CreateMajorRequest } from "@/types/admin/major";
...
export default function MajorCreate() {
  ...
  const [formData, setFormData] = useState<CreateMajorRequest>({
    code: "",
    name: "",
    orgUnitId: undefined,
    isActive: true,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      await api.post("/admin/major", formData);
      ...
      if (status === 409) {
        ...
        toast.error("Không tìm thấy đơn vị tổ chức");
      } else {
        toast.error(err.response?.data?.message || "Không thể tạo ngành học");
      }
    } finally {
      setIsLoading(false);
    }
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Tạo mới ngành đào tạo trong hệ thống
          </p>
        </div>
      </div>
      ...
      <Card className="max-w-2xl">
        <CardHeader>
          ...
          <CardDescription>Nhập thông tin ngành học mới</CardDescription>
        </CardHeader>
        <CardContent>
