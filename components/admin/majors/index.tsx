...
import { MajorPagedResponse } from "@/types/admin/major";
...
export default function Majors() {
  ...
  const loadMajors = async () => {
    ...
    try {
      ...
      const response = await api.get<MajorPagedResponse>(
        `/admin/major?${params}`
      );
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Ngành học</h1>
          <p className="text-muted-foreground mt-1.5">
            Quản lý các ngành đào tạo trong hệ thống
          </p>
        </div>
        <Button asChild>
      ...
      <Card>
        <CardHeader className="pb-4">
          <div className="flex items-center justify-between flex-wrap gap-4">
            <div>
              <CardTitle className="text-xl">Danh sách ngành học</CardTitle>
              <CardDescription className="mt-1">
                {data
                  ? `Tổng cộng ${data.meta.total} ngành học`
                  : "Tất cả ngành học trong hệ thống"}
              </CardDescription>
            </div>
            <MajorFilter
              q={q}
