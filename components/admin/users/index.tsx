...
import { PagedResponse, UserResponse } from "@/types/admin/user";
...
export default function Users() {
  ...
  const loadUsers = async () => {
    ...
    try {
      ...
      const response = await api.get<PagedResponse<UserResponse>>(
        `/admin/user?${params}`
      );
          ...
          <div className="flex items-center justify-between flex-wrap gap-4">
            <div>
              ...
              <CardDescription className="mt-1">
                {data
                  ? `Tổng cộng ${data.meta.total} người dùng`
                  : "Tất cả người dùng đã đăng ký trên hệ thống"}
              </CardDescription>
            </div>
            <UserFilter
              q={q}
