...
import { PagedMeta } from "@/types/admin/notebook";
  ...
  return (
    <div className="flex items-center justify-between flex-wrap gap-4">
      <p className="text-sm text-muted-foreground">
        Hiển thị <span className="font-medium text-foreground">{meta.page * meta.size + 1}</span> -{" "}
        <span className="font-medium text-foreground">
          {Math.min((meta.page + 1) * meta.size, meta.total)}
        </span>{" "}
        trong tổng số <span className="font-medium text-foreground">{meta.total}</span> notebook
      </p>
