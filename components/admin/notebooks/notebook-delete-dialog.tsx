...
export default function NotebookDeleteDialog({
  ...
  const handleDelete = async () => {
    setIsLoading(true);
    try {
      await api.delete(`/admin/community/${notebookId}`);
      toast.success("Xóa notebook thành công");
      onOpenChange(false);
      onDelete();
    } catch (error: any) {
      console.error("Error deleting notebook:", error);
      toast.error(error.response?.data?.message || "Không thể xóa notebook");
    } finally {
      setIsLoading(false);
    }
  };
          ...
          <div className="flex-1 space-y-3">
            <AlertDialogHeader className="text-left">
              <AlertDialogTitle className="text-xl font-semibold">
                Xác nhận xóa notebook
              </AlertDialogTitle>
              <AlertDialogDescription className="text-sm leading-relaxed text-muted-foreground">
                Bạn có chắc chắn muốn xóa notebook{" "}
            ...
            <AlertDialogFooter className="flex-row justify-end gap-2 sm:gap-2 pt-2">
              ...
              >
                {isLoading ? "Đang xóa..." : "Xóa notebook"}
              </AlertDialogAction>
            </AlertDialogFooter>
          </div>
        </div>
