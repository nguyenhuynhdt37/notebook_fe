...
export default function ManualCreateClassFlow({ onBack }: ManualCreateClassFlowProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.className.trim() || !formData.assignmentId || !formData.room.trim() || !formData.periods) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center space-x-4">
        ...
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Tạo lớp học phần</h1>
          <p className="text-muted-foreground">Tạo lớp học phần mới từ môn được phân công</p>
        </div>
      </div>
          ...
          <CardContent className="space-y-4">
            <div className="grid gap-4 md:grid-cols-2">
              <div className="space-y-2 md:col-span-2">
                <Label htmlFor="assignment">Môn phân công *</Label>
                <LecturerAssignmentSelect
                  value={formData.assignmentId || null}
                  onChange={(value) => handleInputChange("assignmentId", value || "")}
